// partially implemented with other functionnality :
// test for brightness using PwmOut
 ...
 */
template <typename T> class AlertCharac : public GattCharacteristic {
public:
  /**
   ...
   */
  AlertCharac(uint8_t properties, const T &initialValue,
                  GattAttribute *descriptors[1] = nullptr,
                  int numOfDescriptors = 0)
      : _initialValue(initialValue),
        GattCharacteristic(0x1802, &_initialValue, sizeof(T), sizeof(T),
  ...
  ble_error_t getValue(const GattServer &server,
                                const GattCharacteristic &characteristic,
                                T &dst) const {
    uint16_t valueSize = sizeof(T);
    return server.read(characteristic.getValueAttribute(), &dst, &valueSize);
  }
